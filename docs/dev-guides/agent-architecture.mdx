...
cd agent
uv sync
uv run fastapi dev app/main.py          # API server
...
These tools call the Open Wearables backend API. All are `async` functions. `user_id` is injected from `RunContext[HealthAgentDeps]` — the model does not pass it.
...
|------|---------|----------------|
| `get_user_profile` | Name, email, birth date, sex, gender | — |
| `get_body_composition` | Weight, height, BMI, body fat, muscle mass, resting HR, HRV | — |
| `get_recent_activity` | Daily steps, distance, calories, active minutes, floors | `days` (1–30, default 7) |
| `get_recent_sleep` | Duration, efficiency, sleep stages, avg HR, HRV, SpO2 | `days` (1–30, default 7) |
| `get_recovery_data` | Resting HR, HRV SDNN, SpO2, sleep efficiency | `days` (1–30, default 7) |
| `get_workouts` | Type, duration, calories, HR stats per session | `days` (1–60, default 14) |
| `get_sleep_events` | Granular stage intervals and timing per sleep session | `days` (1–14, default 7) |
| `get_heart_rate_timeseries` | HR time-series at 1-hour resolution | `hours` (1–168, default 24) |
...
<Tabs>
  ...
  <Tab title="Google">
    ...
    # LLM_MODEL_WORKERS=gemini-1.5-flash
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="API / DB">
    ...
    | `DB_NAME` | `open-wearables` | Database name |
...
The test suite requires Docker (for PostgreSQL via testcontainers) or a running PostgreSQL instance.
...
TEST_DATABASE_URL=postgresql+psycopg://user:pass@localhost:5432/agent_test \
    uv run pytest -v

# Run only the agent unit tests (no DB needed — pure computation)
uv run pytest tests/agent/test_stats_tools.py -v
...
<Tip>
The stats tools and date tools tests (`test_stats_tools.py`, parts of `test_tools.py`) are pure unit tests. They pass even without Docker because they don't touch the database. Run them first to quickly verify logic changes.
</Tip>
...
| `tests/agent/test_tools.py` | All OW data tools — mocked `ow_client`, error handling |
...
2. For tools that need user context: make it `async`, accept `ctx: RunContext[HealthAgentDeps]` as the first argument, and extract `user_id` via `_require_user_id(ctx)`.
...
<AccordionGroup>
  ...
  <Accordion title="Tool calls return 'Error fetching ...'">
    ...
    ```bash
    curl http://$OW_API_URL/api/v1/users/$SOME_USER_ID \
